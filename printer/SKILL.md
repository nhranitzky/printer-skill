...
This skill enables Claude or Openclaw to discover network printers and print files using Python.
...
{baseDir}/bin/printit list_printers
{baseDir}/bin/printit list_printers --json

# Text table — human-readable
{baseDir}/bin/printit list_printers --text
{baseDir}/bin/printit list_printers --text --verbose
```
...
{baseDir}/bin/printit print_file --file /path/to/document.pdf

# Use a specific printer
{baseDir}/bin/printit print_file --file /path/to/document.pdf --printer "HP_LaserJet"

# Print duplex (two-sided)
{baseDir}/bin/printit print_file --file /path/to/document.pdf --duplex

# Print simplex (one-sided, explicit)
{baseDir}/bin/printit print_file --file /path/to/document.pdf --simplex

# Combine options
{baseDir}/bin/printit print_file --file /path/to/document.pdf --printer "HP_LaserJet" --duplex --copies 2
```
...
```bash
# Print a web page
{baseDir}/bin/printit print_url --url https://example.com

# Print in duplex / landscape
{baseDir}/bin/printit print_url --url https://example.com --duplex --landscape

# Use a specific printer and paper size
{baseDir}/bin/printit print_url --url https://news.ycombinator.com --printer "HP_LaserJet" --media Letter

# Scale down wide pages to fit on paper (0.8 = 80%)
{baseDir}/bin/printit print_url --url https://example.com --scale 0.8

# Wait 5 s for animations or lazy-loaded content before rendering
{baseDir}/bin/printit print_url --url https://example.com --wait 5

# Disable network-idle wait (for live dashboards or streaming pages)
{baseDir}/bin/printit print_url --url https://dashboard.example.com --no-network-wait

# Combine options
{baseDir}/bin/printit print_url --url https://example.com --printer "Epson_WF" --duplex --copies 2 --scale 0.9
```
...
{baseDir}/bin/printit print_text --file notes.txt

# Custom font and size
{baseDir}/bin/printit print_text --file notes.txt --font Helvetica --font-size 11

# Monospace code style, small, landscape for wide output
{baseDir}/bin/printit print_text --file output.log --font Courier --font-size 8 --landscape

# Loose line spacing and wider margins
{baseDir}/bin/printit print_text --file readme.txt --line-spacing 1.5 --margin 2.5cm

# Serif font, duplex
{baseDir}/bin/printit print_text --file report.txt --font Times-Roman --font-size 12 --duplex

# Full control
{baseDir}/bin/printit print_text --file notes.txt --printer "HP_LaserJet" --font Helvetica-Bold \
...
### `{baseDir}/bin/printit print_url` key options
...
### `{baseDir}/bin/printit print_text` key options
...
- **Text font looks wrong** → Check font name spelling; run `printit print_text --help` for the full list
